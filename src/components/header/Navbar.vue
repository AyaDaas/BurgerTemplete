...
<nav :class="{ 'bg-white': isScrolled }" class="bg-transparent transition-all duration-300 ease-in-out  w-full fixed  ">
  <div class="max-w-screen-full flex flex-wrap items-center justify-between  mx-auto p-4">
    <a href="https://flowbite.com/" class="flex  text-white  items-center">
        <PhFillForkKnife  class="h-9 mr-3  text-4xl text-white  " />
          <span class="self-center text-white	 text-2xl font-semibold whitespace-nowrap titels-font dark:text-white">logo here</span>
    </a>
    ...
    <div class="hidden  mt-3 bg-yellow-600 md:bg-transparent w-full md:block md:w-auto" id="navbar-default">
      <ul  class="font-medium text-white flex flex-col p-4 md:p-0 m-6   border border-gray-100 rounded-lg md:hover:bg-transparent  bg-transparent  md:flex-row md:space-x-8 md:mt-0 md:border-0   dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
        <li  v-for="item in dataNav" :key="item.label">
          <router-link   :to="item.to"
          @click="setActive(item)"
          :class="{ 'text-yellow-600 md:bg-transparent bg-white': item.active }"   
                  class="block py-2 pl-3 pr-4 hover:bg-white m-1 hover:text-yellow-600 active.text-yellow-600 md:hover:bg-transparent  rounded  md:hover:text-yellow-600  md:p-0 dark:text-white md:dark:text-blue-500" 
           aria-current="page">{{item.label}}</router-link>
        </li>
        <li>
   <svg  xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="m-1 mt-2  bi bi-search md:hover:text-yellow-600 block my-1    mx-3   " viewBox="0 0 16 16">
